...
# Although type systems change, it is a rare event and even if it happens, we should be very careful as way too many things can break.
# For this reason we'll be testing values and members for the types we define to catch any changes.
# For this reason we'll be primarily be testing the values and not the utility functions for the conversion between types.
# These functions are trivial and they primarily expose a thin API over the mappings we define.

# tests for primitive types.
def test_primitivecategory_members():
    expected_members = {
        "VOID", "BOOLEAN", "BYTE", "SHORT", "INT", "LONG", "FLOAT",
        "DOUBLE", "STRING", "DATE", "TIMESTAMP", "TIMESTAMPLOCALTZ",
        "BINARY", "DECIMAL", "VARCHAR", "CHAR", "INTERVAL_YEAR_MONTH",
...
def test_htype_members():
    expected_members = {
        "PRIMITIVE", "STRUCT", "MAP", "LIST", "UNION"
    }
...
def test_htype_values():
    expected_values = {
        PrimitiveCategory, "STRUCT", "MAP", "LIST", "UNION"
    }

    actual_values = {member.value for member in HTypeCategory}
    assert actual_values == expected_values

# Tests for the Serde definitions. Remember that we are converting between Thrift and Hive types so we need to account for that.
# It's important to test that stuff here because the thrift types are auto generated and we should catch any changes that might happen in the .thrift files as soon as possible.
def test_serdetypes_members():
    expected_members = {
        "VOID", "BOOLEAN", "TINYINT", "SMALLINT", "INT", "BIGINT", "FLOAT",
        "DOUBLE", "STRING", "DATE", "CHAR", "VARCHAR", "TIMESTAMP",
        "TIMESTAMPLOCALTZ", "DECIMAL", "BINARY", "INTERVAL_YEAR_MONTH",
        "INTERVAL_DAY_TIME", "LIST", "MAP", "STRUCT", "UNION", "UNKNOWN"
    }
...
def test_serdetypes_values():
    expected_values = {
        "void", "boolean", "tinyint", "smallint", "int", "bigint", "float",
        "double", "string", "date", "char", "varchar", "timestamp",
        "timestamp with local time zone", "decimal", "binary", "interval_year_month",
        "interval_day_time", "array", "map", "struct", "uniontype", "unknown"
    }
...
def test_typeparser_tokenize():
    ...
    parser = TypeParser("STRUCT<houseno:STRING,streetname:STRING,town:STRING,postcode:STRING>")
    assert parser.index == 0
    assert parser.type_tokens == [
        Token(position=0, text="STRUCT", type_=True),
        Token(position=6, text="<", type_=False),
...
def test_decimal_type_parser():
    ...
    assert type(ptype) == HDecimalType
...
def test_char_type_parser():
    ...
    ptype = parser.parse_type()
    assert type(ptype) == HCharType
    assert ptype.length == 10

    #Testing char without length
    parser = TypeParser("char")
    with pytest.raises(ValueError, match="char/varchar type must have a length specified"):
        parser.parse_type()
    ...
    with pytest.raises(ValueError, match="Error: char type takes only one parameter, but instead 2 parameters are found."):
        parser.parse_type()
...
def test_var_char_type_parser():
    ...
    ptype = parser.parse_type()
    assert type(ptype) == HVarcharType
    assert ptype.length == 10
    ...
    with pytest.raises(ValueError, match="Error: varchar type takes only one parameter, but instead 2 parameters are found."):
        parser.parse_type()
...
def test_array_type_parser():
    ...
    ptype = parser.parse_type()
    assert type(ptype) == HListType
    assert ptype.elementType.name == "INT"
    assert ptype.elementType.category.PRIMITIVE == HPrimitiveType(PrimitiveCategory.INT).category

def test_struct_type_parser():
    parser = TypeParser("struct<name:string,age:int>")
    ptype:HStructType = parser.parse_type()
    assert type(ptype) == HStructType
...
def test_map_type_parser():
    ...
    ptype = parser.parse_type()
    assert type(ptype) == HMapType
    assert ptype.keyType.name == "STRING"
    assert ptype.keyType.category == HPrimitiveType(PrimitiveCategory.STRING).category
    assert ptype.valueType.name == "INT"
    assert ptype.valueType.category == HPrimitiveType(PrimitiveCategory.INT).category

def test_union_type_parser():
    parser = TypeParser("uniontype<int,string>")
    ptype = parser.parse_type()
    assert type(ptype) == HUnionType
